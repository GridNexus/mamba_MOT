...
DEFAULT_IMAGE_DIR = "/data1/pengsihaoran/data/dwtest/images/train"
DEFAULT_GT_ROOT = "/data1/pengsihaoran/data/dwtest/gt"
DEFAULT_OUTPUT_DIR = "/data1/pengsihaoran/mamba/outputs/mambavision_mot"
DEFAULT_MAMBAVISION_CONFIG = "/data1/pengsihaoran/mamba/MambaVision-main/object_detection/configs/mamba_vision/mamba_vision_tiny.py"
DEFAULT_MAMBAVISION_CHECKPOINT = "/data1/pengsihaoran/mamba/MambaVision-main/ckp/cascade_mask_rcnn_mamba_vision_tiny_3x_coco.pth"
DEFAULT_MAMBAVISION_SOURCE = "/data1/pengsihaoran/mamba/MambaVision-main"
...
class MambaVisionDetector:
    """MambaVision detector using MMDetection."""
    ...
    def __init__(
        ...
        print(f"MambaVision detector loaded on {self.device}")

    def _load_model(self):
        # Import mamba_vision module to register MM_mamba_vision
        # Add the tools directory to sys.path
        tools_dir = os.path.join(self.mambavision_source_dir, "object_detection", "tools")
        if tools_dir not in sys.path:
            sys.path.insert(0, tools_dir)

        # Import mamba_vision to register MM_mamba_vision
        import mamba_vision
        print("MM_mamba_vision module registered")

        from mmdet.apis import init_detector
        return init_detector(self.config_path, self.checkpoint_path, device='cpu')
    ...
    def detect(self, image_bgr: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if isinstance(result, DetDataSample):
            ...
            for i, (box, score, label) in enumerate(zip(boxes, scores, labels)):
                # Only keep person class (COCO class_id=0) to filter out false positives
            ...
            for i, det in enumerate(result):
                if det.shape[0] > 0:
                    for box in det:
                        ...
                        # Only keep person class (COCO class_id=0) to filter out false positives
                        if i == 0 and score >= self.box_threshold:
                            detections.append({
...
def main() -> None:
    ...
    if args.show_params:
        print("\n=== MambaVision Detector Model ===")
        print_model_params(mot.detector.model, "MambaVision Detector")
        print("\n=== MambaVision ReID Model ===")
        print_model_params(mot.reid.model, "MambaVision ReID")

    print(f"\nProcessing {len(seq_list)} sequence(s)")
    print(f"Images: {image_split_dir}")
    print(f"GT root: {gt_split_dir}")
    print(f"Output: {args.output_dir}")
    print(f"Detector: MambaVision Tiny (Cascade Mask R-CNN)")
    print(f"ReID: MambaVision")
    print(f"Config: {args.mambavision_config}")
